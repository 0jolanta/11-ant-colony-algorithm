...
if __name__ == "__main__":
    ...
    Obstacles = [(0, 2), (0, 3), (0, 10), (0, 11), 
                 ...
                 (17, 3), (17, 6)]
    #aco = AntColonyOptimalization(width, height, obstacles, ant_count, step_count, alpha, beta, gamma, evaporation_rate, start, end, brushfire_iter, colony_iter)
    aco = AntColonyOptimalization(20, 20, Obstacles, 20, 100, 1.1, 12, 0.02, 0.5, np.array([0, 0]), np.array([19, 19]), 5, 20)
    heuristic_grid = aco.heuristic_grid
    plt.figure()
    plt.imshow(heuristic_grid)
    plt.show()
    path = aco.move_ant()
    aco.plot(path)
    path = aco.geometric_path_optimizer(path)
    aco.plot(path)
    o = aco.run_ant_colony(10)
    aco.plot(o[0][1])
